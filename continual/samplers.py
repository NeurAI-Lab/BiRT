...
class SingleRASampler(torch.utils.data.Sampler):
    ...
    def __iter__(self):
        ...
        indices = [ele for ele in indices for i in range(self.num_replicas)]
...
def get_sampler(dataset_train, dataset_val, args):
    if args.distributed:
        ...
        if args.use_repeatedaug_single:
            sampler_train = SingleRASampler(dataset_train, num_replicas=2, rank=0, shuffle=True)
        else:
