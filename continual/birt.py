...
class BiRT(nn.Module):
    ...
    def __init__(
        ...
        joint_tokens=False,
        num_blocks=None,
        multi_token_setup=False
    ):
        super().__init__()

        self.nb_classes = nb_classes # 2
        self.embed_dim = transformer.embed_dim
        self.individual_classifier = individual_classifier
        self.use_head_div = head_div # true
        self.head_div_mode = head_div_mode # tr
        self.head_div = None
        self.joint_tokens = joint_tokens # False
        self.in_finetuning = False
        self.multi_token_setup = multi_token_setup
        ...
        self.sabs = transformer.blocks[:-1]

        self.tabs = transformer.blocks[-1:]

        self.task_tokens = nn.ParameterList([transformer.cls_token])

        if self.individual_classifier != '':
            in_dim, out_dim = self._get_ind_clf_dim() # 384, 10
    ...
    def add_model(self, nb_new_classes, multi_token_setup=False):
        ...
        if self.use_head_div:
            self.head_div = ContinualClassifier(
                self.sabs[-1].dim, self.nb_classes_per_task[-1] + 1
            ).cuda()
        # ----------------------------------------------------------------------

        # Classifier -----------------------------------------------------------
        if self.individual_classifier != '' and not multi_token_setup:
            in_dim, out_dim = self._get_ind_clf_dim()
            self.head.append(
                ContinualClassifier(in_dim, out_dim).cuda()
            )
        elif not multi_token_setup:
            self.head = ContinualClassifier(
                self.embed_dim * len(self.task_tokens), sum(self.nb_classes_per_task)
            ).cuda()
        # ----------------------------------------------------------------------
    ...
    def freeze(self, names):
        ...
        for name in names:
            ...
            elif name == 'partial_sab':
                cutils.freeze_parameters(self.patch_embed, requires_grad=requires_grad)
                cutils.freeze_parameters(self.pos_embed, requires_grad=requires_grad)
                cutils.freeze_parameters(self.sabs[:self.num_blocks], requires_grad=requires_grad)
            elif name == 'tab':
                self.tabs.eval()
    ...
    def forward_initial(self, x):
        ...
        for blk in self.sabs[:self.num_blocks]:
            x, attn, v = blk(x)

        return x

    def forward_latter(self, x, args=None):
        B = x.shape[0]
        s_e, s_a, s_v = [], [], []
        for blk in self.sabs[self.num_blocks:]:
            x, attn, v = blk(x, args=args)
            s_e.append(x)
            s_a.append(attn)
            s_v.append(v)
        ...
        return self.forward_classifier(tokens, tokens[-1], attentions)

    def forward_features_multitoken(self, x, batch_tasks=None):
        # Shared part, this is the ENCODER
        B = x.shape[0]
        ...
        if self.training:
            x_tokens = torch.cat((torch.cat([self.task_tokens[i] for i in batch_tasks]), x), dim=1)
            ca_blocks = self.tabs
            for blk in ca_blocks:
                task_token, attn, v = blk(x_tokens, mask_heads=mask_heads)

            attentions.append(attn.unsqueeze(dim=0))
            tokens.append(task_token[:, 0])
    ...
    def forward_features(self, x, batch_tasks=None):
        # Shared part, this is the ENCODER
        B = x.shape[0]
    ...
    def forward_classifier_multitoken(self, tokens, last_token, attentions):
        ...
        # assuming self.individual_classifier is always '1-1' here
        for i in range(len(tokens)):
            logits.append(self.head[0](tokens[i]))
        ...
        return {
            'logits': logits,
            'div': logits_div,
            'tokens': tokens,  # 128, 384
            'attention': attentions  # 128, 12, 1, 65
        }

    def forward_classifier(self, tokens, last_token, attentions):
        """Once all task embeddings e_1, ..., e_t are extracted, classify.
        ...
        # modify attentions list to extract only the first element
        attentions = torch.cat(attentions, dim=0)
    ...
    def forward(self, x, batch_tasks=None, initial=False, latter=False, args=None):
        if initial:
            return self.forward_initial(x)
        elif latter:
            return self.forward_latter(x, args=args)
        elif self.multi_token_setup:
            tokens, last_token, attentions = self.forward_features_multitoken(x, batch_tasks=batch_tasks)
            return self.forward_classifier_multitoken(tokens, last_token, attentions)
        else:
            tokens, last_token, attentions = self.forward_features(x, batch_tasks=batch_tasks)
            return self.forward_classifier(tokens, last_token, attentions)
