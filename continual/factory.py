...
from continual import convit, birt, samplers, vit
...
def get_loaders(dataset_train, dataset_val, args, drop_last=True):
...
def update_birt(model_without_ddp, task_id, args):
    if task_id == 0:
        print(f'Creating BiRT!')
        model_without_ddp = birt.BiRT(
            model_without_ddp,
            nb_classes=args.initial_increment, # 10
            individual_classifier=args.ind_clf, #1-1
            head_div=args.head_div > 0., # 0.1
            head_div_mode=args.head_div_mode, # 'tr'
            joint_tokens=args.joint_tokens, # False
            num_blocks=args.replay_from, # from which block to replay representation
            multi_token_setup=args.multi_token_setup,
        )
    else:
        print(f'Updating ensemble, new embed dim {model_without_ddp.sabs[-1].dim}.')
        model_without_ddp.add_model(args.increment, args.multi_token_setup)
