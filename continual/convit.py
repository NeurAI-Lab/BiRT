...
class GPSA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
                 locality_strength=1., use_local_init=True, fc=None, dynamic_tokens=1.0, attn_version='v1'):
        super().__init__()
    ...
    def forward(self, x, args=None):
        ...
        # augmenting the attention map based on different types of noise
        if args:
            if args.rep_noise_weight > 0. and args.repnoise_prob > 0:
                aug_indexes = torch.rand(attn.shape[0]) < args.repnoise_prob
                attn[aug_indexes] += torch.normal(mean=0, std=args.rep_noise_weight,
                                                  size=attn[aug_indexes].shape,
                                                  device='cuda')

            if args.shotnoise_strength > 0. and args.shot_prob > 0:
                # normalize representations
                rep_min = attn.min()
                rep_max = attn.max()
                attn = (attn + torch.abs(rep_min)) / (rep_max - rep_min)

                aug_indexes = torch.rand(attn.shape[0]) < args.shot_prob
                attn[aug_indexes] = torch.poisson(attn[aug_indexes] * \
                                                  args.shotnoise_strength) / \
                                                  torch.tensor(args.shotnoise_strength, dtype=torch.float)

                attn = (attn * (rep_max - rep_min)) - torch.abs(rep_min)

            if args.tensor_noise_weight > 0. and args.tensor_prob > 0:
                aug_indexes = torch.rand(attn.shape[0]) < args.tensor_prob
                attn[aug_indexes] += args.tensor_noise_weight * torch.normal(mean=0,
                                                                             std=float(torch.std(attn)),
                                                                             size=attn[aug_indexes].shape,
                                                                             device='cuda')

            if args.scale_prob > 0:
                aug_indexes = torch.rand(attn.shape[0]) < args.scale_prob
                attn[aug_indexes] *= torch.arange(0.95, 1.2, 0.001)[torch.randint(11, (1,))].cuda()
...
class MHSA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
                 fc=None, dynamic_tokens=1.0, attn_version='v1'):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = qk_scale or head_dim ** -0.5
...
class LatterBlock(nn.Module):
    ...
    def forward(self, x, mask_heads=None, task_index=1, attn_mask=None, args=None):
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads,  mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, attention_type=GPSA,
                 fc=nn.Linear, dynamic_tokens=False, attn_version='v1', **kwargs):
        super().__init__()
        self.dim = dim
        self.norm1 = norm_layer(dim)
        self.attn = attention_type(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
                                   proj_drop=drop, fc=fc, dynamic_tokens=dynamic_tokens,
                                   attn_version=attn_version, **kwargs)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, fc=fc)
    ...
    def forward(self, x, mask_heads=None, task_index=1, attn_mask=None, args=None):
        ...
        xx = self.norm1(x)
        xx, attn, v = self.attn(xx, args=args)
...
class ClassAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., fc=nn.Linear,
                 dynamic_tokens=1.0, attn_version='v1'):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = qk_scale or head_dim ** -0.5
    ...
    def forward(self, x, mask_heads=None, attn_version='v1', **kwargs):
        B, N, C = x.shape
        ...
        attn = attn.softmax(dim=-1) # 128, 12, 1, 65

        ret_attn = None
        if self.attn_version == 'v1':
            ret_attn = attn
        elif self.attn_version == 'v2':
            ret_attn = attn[:, :, :, 1:]
        elif self.attn_version == 'v3':
            ret_attn = torch.squeeze(torch.mean(attn, dim=1))
        elif self.attn_version == 'v4':
            ret_attn = torch.squeeze(torch.sum(attn, dim=1))

        attn = self.attn_drop(attn)

        if self.dynamic_tokens < 1.:
            topk_num = int(attn.shape[-1]*self.dynamic_tokens)
            vals, ids = torch.topk(attn, topk_num, dim=3)
            new_attn = torch.zeros_like(attn)
            new_attn.scatter_(-1, ids, vals)
        else:
            new_attn = attn

        if mask_heads is not None:
            mask_heads = mask_heads.expand(B, self.num_heads, -1, N)
            new_attn = new_attn * mask_heads

        x_cls = (new_attn @ v).transpose(1, 2).reshape(B, 1, C)
        x_cls = self.proj(x_cls)
        x_cls = self.proj_drop(x_cls)

        return x_cls, ret_attn, v
...
class ConVit(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 class_attention=False, ca_type='base', dynamic_tokens=False, num_blocks=1, attn_version='v1'
        ):
        super().__init__()
        self.num_classes = num_classes #100
        self.num_heads = num_heads # 12
        self.embed_dim = embed_dim # 384
        self.local_up_to_layer = local_up_to_layer # 5
        ...
        # initially ca type is base, so class attention
        if ca_type == 'base':
        ...
        for layer_index in range(depth):
            if layer_index < local_up_to_layer and layer_index+1 == self.num_blocks:
                # Convit
                initial_block = InitialBlock(
                    ...
                    attention_type=GPSA, locality_strength=locality_strength, dynamic_tokens=dynamic_tokens,
                )
                latter_block = LatterBlock(
                    dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[layer_index], norm_layer=norm_layer,
                    attention_type=GPSA, locality_strength=locality_strength, dynamic_tokens=dynamic_tokens,
                )
                block = [initial_block, latter_block]
            elif layer_index < local_up_to_layer and layer_index+1 != self.num_blocks:
                # Convit
                block = Block(
                    dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[layer_index], norm_layer=norm_layer,
                ...
                block = Block(
                    dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[layer_index], norm_layer=norm_layer,
                    attention_type=MHSA, dynamic_tokens=dynamic_tokens,
                )
                block = [block]
            else:
                # CaiT
                block = Block(
                    dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[layer_index], norm_layer=norm_layer,
                    attention_type=ca_block, dynamic_tokens=dynamic_tokens, attn_version=attn_version
                )
                block = [block]
        ...
        self.use_class_attention = class_attention  # true

        # Classifier head
        self.feature_info = [dict(num_chs=embed_dim, reduction=0, module='head')]
        self.head = nn.Linear(embed_dim, num_classes) if num_classes > 0 else nn.Identity() # directly 100 classes
